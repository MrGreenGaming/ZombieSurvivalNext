...
function GM:AddResources()
	...
	resource.AddFile("resource/fonts/zsnew.ttf")
	...
	resource.AddFile("materials/blood_decals/blood_splat01.vmt")
	resource.AddFile("materials/blood_decals/blood_splat02.vmt")
	resource.AddFile("materials/blood_decals/blood_splat03.vmt")
	resource.AddFile("materials/blood_decals/blood_splat04.vmt")
	resource.AddFile("materials/blood_decals/blood_splat05.vmt")
	resource.AddFile("materials/blood_decals/blood_splat06.vmt")
	resource.AddFile("materials/blood_decals/blood_splat07.vmt")
	resource.AddFile("materials/blood_decals/blood_splat08.vmt")
...
function GM:SetupSpawnPoints()
	...
	if string.sub(mapname, 1, 3) == "cs_" or string.sub(mapname, 1, 3) == "zs_" or string.sub(mapname, 1, 3) == "za_"then
...
function GM:InitPostEntity()
	...
	if string.find(string.lower(GetConVarString("hostname")), "hellsgamers", 1, true) then
		...
		ErrorNoHalt("You are also a load of cunts ;)")
	end
end
...
function GM:LoadNextMap()
	...
	if file.Exists(GetConVarString("mapcyclefile"), "GAME") then
		...
		maps = table.Add(maps, file.Find("maps/za_*.bsp", "GAME"))
		table.sort(maps)
		if #maps > 0 then
...
util.AddNetworkString( "zs_gamestart" )
function GM:PlayerInitialSpawn(pl)
	--timer.Simple(5, function() 
		--util.AddNetworkString( "zs_gamestart" )
		--net.Start("zs_gamestart")--Send the notification to play the round start music
	--net.Send(pl)
	--end)
	gamemode.Call("PlayerInitialSpawnRound", pl)
end
...
function GM:PlayerHurt(victim, attacker, healthremaining, damage)
	...
	if victim:Team() == TEAM_HUMAN then
		...
		if healthremaining < 75 and 1 <= healthremaining then
			--victim:ResetSpeed(nil, healthremaining) --Test!
		end
	end
...
function GM:PlayerSpawn(pl)
	...
	if pl:Team() == TEAM_UNDEAD then
		...
		elseif self.Arena then	
			pl:Give(table.Random(self.ArenaModeWeapons))
			net.Start( "arena" ) --Net message for Arena Stuff......
			net.WriteEntity( pl)
			net.Broadcast()
...
if pl:Team() == TEAM_HUMAN then --HUMANS DICE
	...
	elseif choice == 3 then --HEALTH
		local calchealth = math.Clamp ( 100 - pl:Health(),25,50 )
		local randhealth = math.random( 25, math.Round ( calchealth ) )
			
		pl:SetHealth( math.min( pl:Health() + randhealth, 100 ) )
		timer.Simple(0.3, function()
			PrintMessage( HUD_PRINTTALK, "WIN: ".. message .." rolled the dice and gained ".. randhealth .." HP!" )
		end)
	elseif choice == 4 then --YOUR SEX IS ON FIRE!

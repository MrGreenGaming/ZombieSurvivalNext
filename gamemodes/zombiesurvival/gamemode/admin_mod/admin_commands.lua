		...
		if (text == "!kill" or text == "!suicide" or text == "!boom") then
			if pl:Alive() and not ENDROUND and GAMEMODE:CanPlayerSuicide(pl) then
				...
				if math.random(1,3) == 1 then
					pl:ChatPrint(suicidenote[math.random(1,#suicidenote)], 2)
				end
			end
...
local function AdminSay(pl, text, teamonly)
	...
	if (#sep > 1 and text:sub(1,1) == "!") then
		...
		if (target == -1) then
			pl:ChatPrint("Target player not found")
			return ""
		elseif (target == -2) then
			pl:ChatPrint("Multiple targets found, please refine your request")
			return ""
		end
		...
		elseif (sep[1] == "!ip") or (sep[1] == "!host") then
			...
			pl:ChatPrint(target:Nick().."'s IP address is ".. target:IPAddress())
			return ""
		end
		
		-- Check if he's alive
		if not target:Alive() or not target:IsValid() then
			pl:ChatPrint("Target player is dead")
			return ""
		end
		...
		if (sep[1] == "!redeem" and target:Team() == TEAM_UNDEAD) then	
			if (target == pl and not pl:IsSuperAdmin()) then
				...
				if not LASTHUMAN or not ENDROUND then
					pl:ChatPrint(target:GetName() .." redeemed")
				else
					pl:ChatPrint("You can't redeem ".. target:GetName() .." at this moment")
				end
				target:ChatPrint("You are redeemed by ".. pl:GetName())
			end
			return ""
		--Slay command
		...
		elseif sep[1] == "!changeclass" and pl:IsSuperAdmin() then
			...
			if type(classId) ~= "number" or not ZombieClasses[classId] then
				pl:ChatPrint("Class doesn't exist")
				return ""
			end
		...
		elseif(sep[1] == "!slap") then
			...
			pl:ChatPrint("Slapped "..target:Name().." with "..slapdam.." damage")
			target:ChatPrint("Admin "..pl:Name().." slapped you with "..slapdam.." damage")
			return ""		
		...
		elseif sep[1] == "!ammo+" and pl:IsSuperAdmin() then
			pl:ChatPrint(target:Name().." was given 100 ammo for his current weapon")
			if (target ~= pl) then
				target:ChatPrint(pl:Name().." has given you 100 ammo for your current weapon")
			end
		...
		elseif (sep[1] == "!hp+" or sep[1] == "!hp") and pl:IsSuperAdmin() then
			...
			pl:ChatPrint("Player "..target:Name().." restored to ".. healthToSet .." health")
			if (target ~= pl) then
				target:ChatPrint(pl:Name().." has set your health to ".. healthToSet)
			end
		...
		elseif sep[1] == "!sp" and pl:IsSuperAdmin() and target:Team() == TEAM_SURVIVORS then
			...
			pl:ChatPrint(target:GetName() .." received ".. amountToGive .." SP")
		...
		elseif sep[1] == "!swep" then -- gives the admin the ability to appoint a swep to a player     
			...
			if not pl:IsSuperAdmin() and target == pl then
				pl:ChatPrint("Only Super Admins can give other admins or themselves weapons")
				return ""
			end
			...
			if (weaponToGive ~= nil) then
				...
				target:ChatPrint("You received ".. weaponToGive .." from ".. pl:GetName())
				pl:ChatPrint(target:GetName() .." was given ".. weaponToGive)
			else
				pl:ChatPrint("Multiple or invalid weapon specified")
			end
			return ""
		elseif (sep[1] == "!--Debug") then
			...
			pl:ChatPrint("--Debug information printed to console")
			return ""
		end
